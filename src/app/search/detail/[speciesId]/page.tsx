...
import {
  ...
  faHouse
} from '@fortawesome/free-solid-svg-icons'; // 引入所有需要的图标

import styles from './page.module.css'; // 页面布局样式
import sectionStyles from '@/search/detail/components/InfoSection.module.css'; // 引入 InfoSection 的 CSS Module
import tagStyles from '@/search/detail/components/Tag.module.css'; // 引入 Tag 的 CSS Module
...
import { SpeciesDetailData } from '@/search/detail/types'; // 引入数据类型
...
// --- Helper Function to get Tag Type --- (简化版)
...
// --- The Page Component ---
// 使用 async 关键字，因为我们在服务器组件中获取数据
export default async function SpeciesDetailPage({ params }: { params: { speciesId: string } }) {

  // 规范化 ID，例如将 URL slug 转换为用于查找的 ID
  await params; // 形式上等待 params
  const normalizedId = params.speciesId.toLowerCase();
  ...
  if (!speciesData) {
    return (
      <div className={styles.container}>
        <p>未找到 ID 为 "{params.speciesId}" 的物种信息。</p>
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.detailContentGrid}>
        ...
        <DetailHeader
          ...
          // icon={faIconMap[speciesData.iconClass] || faMicroscope} // 可以根据数据动态选择图标
        />
        ...
        <div className={styles.mainInfoColumn}>
                ...
                <div className={tagStyles.tagContainer}>
                    {speciesData.transmission.mediums.map((medium, index) => (
                         <Tag
                            ...
                            tooltip={`${medium.type === 'Vector' ? '传播媒介' : ''} ${medium.method}`}
                        />
                    ))}
                </div>
          ...
          {speciesData.references && speciesData.references.length > 0 && (
             <InfoSection title="相关文献引用" icon={faBookOpen}>
                <ul className={styles.referenceList}> {/* 使用特定样式类 */}
                    {speciesData.references.map(ref => (
                        <li key={ref.id}>
                            ...
                            {ref.doi && <span className={styles.doiInfo}>DOI: {ref.doi}</span>}
                            {ref.link && <Link href={ref.link} target="_blank" className={styles.refLink}><FontAwesomeIcon icon={faAngleRight} /> 查看链接</Link>}
                            {/* PDF 链接示例 */}
                            {/* {ref.pdfPath && <Link href={ref.pdfPath} target="_blank" className={styles.refFileLink}><FontAwesomeIcon icon={faFilePdf} /> 下载PDF</Link>} */}
                        </li>
                    ))}
                </ul>
                {/* 可以添加查看所有文献的链接 */}
                {/* <Link href={`/literature?speciesId=${speciesData.id}`} className={sectionStyles.viewAllLink}>
                    查看所有关联文献 <FontAwesomeIcon icon={faAngleRight} />
                </Link> */}
             </InfoSection>
          )}


        </div> {/* End Left Main Info Column */}
        ...
        <aside className={styles.sideInfoColumn}>
            ...
            <InfoSection title="快速概览" icon={faBolt} className={styles.stickyCard}>
                <dl className={styles.quickFactsList}>
                    ...
                    <div><dt><FontAwesomeIcon icon={faCheckCircle} />确认状态:</dt><dd className={`${styles.statusText} ${styles['status'+speciesData.status.charAt(0).toUpperCase() + speciesData.status.slice(1)]}`}>{speciesData.statusText}</dd></div>
            ...
            {speciesData.images && speciesData.images.length > 0 && (
                <InfoSection title="相关图片" icon={faImages} className={styles.stickyCard}>
                    ...
                    {/* 可以添加查看全部图片的链接 */}
                    {/* <Link href={`/gallery?speciesId=${speciesData.id}`} className={sectionStyles.viewAllLink}>
                        查看全部图片 <FontAwesomeIcon icon={faAngleRight} />
                    </Link> */}
                </InfoSection>
            )}
        ...
        </aside> {/* End Right Sidebar Column */}
      </div> {/* End Detail Content Grid */}
    </div>
  );
}